...
const AudioSubmission: React.FC = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Call API
            await sessionService.upload(airportCode, sessionDate, filesToUpload);

            // On success
            navigate('/atco-dashboard');
        } catch (err: any) {
            console.error(err);
                ...
                <div className="relative w-full max-w-3xl flex flex-col bg-surface-dark border border-surface-border rounded-xl shadow-2xl overflow-hidden animate-fade-in-up">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <label className="flex flex-col gap-2">
                                ...
                                <div className="relative">
                                    ...
                                    <select
                                        className="w-full bg-[#111814] border border-surface-border text-white text-sm rounded-lg pl-11 pr-10 py-3 focus:ring-1 focus:ring-primary focus:border-primary appearance-none cursor-pointer hover:border-text-secondary transition-colors"
                                        value={airportCode}
                                        onChange={(e) => setAirportCode(e.target.value)}
                                    >
                                        <option disabled value="">Select Airport (ej. LECU)...</option>
                                        <option value="LECU">LECU - Madrid Cuatro Vientos</option>
                                        <option value="GCFV">GCFV - Fuerteventura</option>
                        ...
                        <div className="group relative flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-surface-border bg-white/[0.02] px-6 py-10 transition-all hover:border-primary/50 hover:bg-white/[0.04]">
                            ...
                            <button className="mt-6 flex items-center justify-center rounded-lg bg-surface-border px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-[#3c5345] focus:ring-2 focus:ring-primary/50 pointer-events-none">
                                Browse Files
                            </button>
                            <input
                        ...
                        {selectedFiles.length > 0 && (
                            <div className="space-y-4">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-sm font-medium text-white">Upload Queue ({selectedFiles.length})</h3>
                                </div>
                                {selectedFiles.map((fileObj, idx) => (
                                    <div key={idx} className="flex items-center gap-4 rounded-lg bg-[#111814] border border-surface-border p-3 transition-colors hover:border-primary/30">
                                        ...
                                        <div className="flex flex-1 flex-col justify-center min-w-0">
                                            <div className="flex justify-between items-center mb-1">
                                                <p className="truncate text-sm font-medium text-white">{fileObj.file.name}</p>
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-xs text-text-secondary shrink-0">{formatSize(fileObj.file.size)}</span>
                                            </div>
                                        </div>
                                        <button onClick={() => removeFile(idx)} className="ml-2 flex h-8 w-8 shrink-0 items-center justify-center rounded-md text-text-secondary hover:bg-white/10 hover:text-red-400 transition-colors">
                    ...
                    <div className="flex items-center justify-end gap-3 px-6 py-5 border-t border-surface-border bg-[#15251d]">
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 rounded-lg bg-primary px-6 py-2.5 text-sm font-bold text-[#052e16] shadow-lg shadow-primary/20 hover:bg-[#15d160] hover:shadow-primary/30 active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <span className="material-symbols-outlined text-[18px] font-bold">{isSubmitting ? 'upload' : 'upload_file'}</span>
